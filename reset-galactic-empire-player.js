...
async function resetPlayer() {
  ...
  try {
    console.log(`🔄 Сброс данных игрока ${telegramId} в Galactic Empire...`);
    console.log('   (основной игрок НЕ удаляется, только данные из GE)\n');
    ...
    `, [telegramId]);
    console.log(`   ✓ Удалено боёв: ${battles.rowCount}`);
    ...
    const logins = await pool.query(`
      ...
      WHERE player_id = $1
    `, [telegramId]);
    console.log(`   ✓ Удалено логинов: ${logins.rowCount}`);
    ...
    const loot = await pool.query(`
      ...
      WHERE player_id = $1
    `, [telegramId]);
    console.log(`   ✓ Удалено лута: ${loot.rowCount}`);
    ...
    const ships = await pool.query(`
      ...
      WHERE player_id = $1
    `, [telegramId]);
    console.log(`   ✓ Удалено кораблей: ${ships.rowCount}`);
    ...
    const formations = await pool.query(`
      ...
      WHERE player_id = $1
    `, [telegramId]);
    console.log(`   ✓ Удалено формаций: ${formations.rowCount}`);
    ...
    const player = await pool.query(`
      ...
      WHERE telegram_id = $1
    `, [telegramId]);
    console.log(`   ✓ Сброшены данные игрока GE: ${player.rowCount}`);

    await pool.query('COMMIT');

    console.log('\n✅ Данные игрока в Galactic Empire сброшены!');
    console.log('   Основной игрок остался в системе.');
    console.log('   Теперь можно заново выбрать расу.\n');
