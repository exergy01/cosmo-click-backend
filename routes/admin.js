...
// ========================
// 📅 ПЛАНИРОВЩИК ЗАДАНИЙ - API ENDPOINTS
// Добавить в конец файла admin.js ПЕРЕД module.exports = router;
// ========================

// 📅 GET /api/admin/scheduler/overview/:telegramId - Обзор планировщика
router.get('/scheduler/overview/:telegramId', async (req, res) => {
  try {
    const { telegramId } = req.params;
...
// 📅 POST /api/admin/scheduler/create-schedule/:telegramId - Создать расписание
router.post('/scheduler/create-schedule/:telegramId', async (req, res) => {
  try {
    const { telegramId } = req.params;
    const { 
      quest_key, 
...
// 📅 POST /api/admin/scheduler/toggle-schedule/:telegramId - Включить/выключить расписание
router.post('/scheduler/toggle-schedule/:telegramId', async (req, res) => {
  try {
    const { telegramId } = req.params;
...
// 📅 GET /api/admin/scheduler/list/:telegramId - Список всех расписаний
router.get('/scheduler/list/:telegramId', async (req, res) => {
  try {
    const { telegramId } = req.params;
...
// 📅 POST /api/admin/scheduler/test-activation/:telegramId - Тестовая активация
router.post('/scheduler/test-activation/:telegramId', async (req, res) => {
  try {
    const { telegramId } = req.params;
...
// 📅 GET /api/admin/scheduler/history/:telegramId - История планировщика
router.get('/scheduler/history/:telegramId', async (req, res) => {
  try {
    const { telegramId } = req.params;
